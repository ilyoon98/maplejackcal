...
// 항목 키(potentialMeso 등) → cube_option_data.js의 큐브 키로 바꿔서 확률 구간을 찾는다
function findBracket(cube, part, level){ return bracketOf(dataKey(cube), part, level); }
...
function renderCost(){
  ...
  $('costInfo').innerHTML = meso
    ? `메소 재설정 1회 <strong>${fmt(mesoResetPrice(state.cube))}</strong> 메소 <span class="tiny">(레전드리 · ${bracket} 구간)</span>`
      + `<br><span class="tiny">큐브를 쓰면 1회 ${fmt(cubeFee(state.level))} 메소</span>`
